"""Cached-latent diagnostic CLI for CogKit CogVideoX5B-RolloutRemover."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CogKit-native cached CogVideoX5B-RR inference"
    )
    ...
    parser.add_argument(
        "--scheduler",
        choices=tuple(SCHEDULER_CLASSES),
        required=True,
    ...
    parser.add_argument(
        "--save_trajectory", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--save_raw_latents", action=argparse.BooleanOptionalAction, default=False
    )
...
def _validate_training_contract(
    ...
    if training.get("method") != "RolloutRemover" or training.get("backbone") != "CogVideoX-5B":
        raise ValueError("training contract is not CogVideoX5B RolloutRemover")
    ...
    if not isinstance(trajectory, dict):
        raise ValueError("training contract has no trajectory")
    expected_trajectory = preset.to_dict()
    for key in ("total_frames", "frame_roles", "clean_condition_indices"):
        if trajectory.get(key) != expected_trajectory[key]:
            raise ValueError(f"training trajectory disagrees on {key}")
...
def _load_explicit_scheduler(
    model_path: Path,
    scheduler_kind: str,
) -> tuple[object, dict[str, Any]]:
    scheduler_class = SCHEDULER_CLASSES[scheduler_kind]
    ...
    if scheduler.__class__ is not scheduler_class:
        raise TypeError(
            f"scheduler class drift: expected {scheduler_class.__name__}, "
...
def main() -> None:
    ...
    _validate_training_contract(args, training, preset)
    scheduler, scheduler_metadata = _load_explicit_scheduler(
        args.model_path,
        args.scheduler,
    )
    ...
    run_contract = {
        ...
        "task": "CogVideoX5B-RolloutRemover-cached-inference",
        ...
        "sample_ids_file": (
            {
                "path": str(args.sample_ids_file),
                "sha256": sha256_file(args.sample_ids_file),
            }
    ...
    if pipeline.scheduler.__class__.__name__ != scheduler_metadata["class_name"]:
        raise RuntimeError("loaded pipeline scheduler disagrees with explicit contract")
    ...
    prompt_embeds, negative_prompt_embeds = encode_rollout_prompt(
        pipeline,
        prompt=args.prompt,
        negative_prompt=args.negative_prompt,
        guidance_scale=args.guidance_scale,
    ...
    for sample_id in args.sample_ids:
        ...
        latents = sample_rollout_latents(
            pipeline,
            preset=preset,
        ...
        generated_indices = (
            list(range(preset.total_frames))
            if args.save_trajectory
            else [preset.mask_prediction_index, preset.final_gt_index]
        )
        generated_decoded = decode_rollout_states_individually(
            pipeline.vae,
        ...
        reference_decoded = decode_rollout_states_individually(
            pipeline.vae,
            reference_latents,
        )[0]
        ...
        mask_prediction_path = sample_dir / "mask_prediction.png"
        prediction_path = sample_dir / "prediction.png"
        raw_latents_path = sample_dir / "raw_f7_f13_latents.pt"
        reference_frames[0].save(mask_path)
        reference_frames[1].save(source_path)
        reference_frames[2].save(gt_path)
        generated_frames[preset.mask_prediction_index].save(mask_prediction_path)
        generated_frames[preset.final_gt_index].save(prediction_path)
        if args.save_raw_latents:
            torch.save(
                {
                    "schema_version": 1,
                    "sample_id": sample_id,
                    "frame_indices": [preset.noisy_anchor_index, preset.final_gt_index],
                    "frame_roles": [
                        preset.frame_roles[preset.noisy_anchor_index],
                        preset.frame_roles[preset.final_gt_index],
                    ],
                    "latents": latents[
                        0, [preset.noisy_anchor_index, preset.final_gt_index]
                    ].detach().cpu(),
                },
                raw_latents_path,
            )
        ...
        if args.save_trajectory:
            trajectory_dir = sample_dir / "trajectory"
            trajectory_dir.mkdir()
            for index, role in enumerate(preset.frame_roles):
                frame_path = trajectory_dir / f"{index:02d}_{role}.png"
                generated_frames[index].save(frame_path)
                trajectory_paths.append(str(frame_path.relative_to(args.output_dir)))
        ...
        sample_payload = {
            ...
            "mask_prediction": str(mask_prediction_path.relative_to(args.output_dir)),
            ...
            "trajectory_frames": trajectory_paths,
            "raw_latents": (
                str(raw_latents_path.relative_to(args.output_dir))
                if args.save_raw_latents
                else None
            ),
            "pure_output": True,
