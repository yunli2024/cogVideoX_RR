...
CHECKPOINT=2200
COGKIT_CWD=/mnt/cpfs/jiachengliu/code/CogKit
SCRIPT_ROOT=${COGKIT_CWD}/quickstart/scripts/rolloutremover_cogvideox
INFERENCE_AUDIT=${SCRIPT_ROOT}/audit_cogkit_rr_rord280_full_8gpu_run_20260808_004018.py
BASE_AUDIT=${SCRIPT_ROOT}/audit_cogkit_rr_rord280_full_20260807.py
EVAL_AUDIT=${SCRIPT_ROOT}/audit_cogkit_rr_rord280_eval_run_20260808_004018.py
...
PIPELINE_LOG=/mnt/cpfs/jiachengliu/dataset/CORNE/logs/cogkit_rr_rord280_full_dpm_8gpu_v1/${RUN_ID}_eval_pipeline.log
...
test ! -e "${PIPELINE_LOG}"
test "$(sha256sum "${INFERENCE_AUDIT}" | awk '{print $1}')" = 4e63b6131b86c19be07a815ebc1e6f1bb8e08285b61e36265a32d25ba4a538be
test "$(sha256sum "${BASE_AUDIT}" | awk '{print $1}')" = fb5b37cae073856089bc5be80b7d1ef26022e8f661cb433e8947b46d661bd126
test "$(sha256sum "${EVAL_AUDIT}" | awk '{print $1}')" = 7c8fabbe3908cd5a937d72a9e6f6d6ff51d89eca3373ee8f8535b57248efadd3
...
gpu_idle() {
  local gpu_id=$1
  local state memory utilization total
  state=$(nvidia-smi -i "${gpu_id}" --query-gpu=memory.total,memory.used,utilization.gpu --format=csv,noheader,nounits)
  IFS=',' read -r total memory utilization <<< "${state}"
  total=${total// /}
  memory=${memory// /}
  utilization=${utilization// /}
  [[ "${total}" -ge 79000 && "${memory}" = 0 && "${utilization}" = 0 ]]
}
...
eval_gpu=""
idle_streak=(0 0 0 0 0 0 0 0)
while [[ -z "${eval_gpu}" ]]; do
  for gpu_id in 7 6 5 4 3 2 1 0; do
    if gpu_idle "${gpu_id}" && gpu_without_compute_process "${gpu_id}"; then
      idle_streak[${gpu_id}]=$((idle_streak[${gpu_id}] + 1))
      if [[ "${idle_streak[${gpu_id}]}" -ge 2 ]]; then
        eval_gpu=${gpu_id}
        break
      fi
    else
      idle_streak[${gpu_id}]=0
    fi
  done
  if [[ -z "${eval_gpu}" ]]; then
    echo "waiting for a verified-idle 80GB GPU; no occupied process will be modified"
