"""Direct-target endpoint adapter over CogKit's native CogVideoX trainer."""
...
from .args import CogVideoXEndpointArgs
...
class CogVideoXEndpointT2VLoraTrainer(CogVideoXT2VLoraTrainer):
    """Keep mask/source conditions and supervise only the direct target frame."""

    @override
    def _init_args(self, uargs_fpath: Path) -> CogVideoXEndpointArgs:
        return CogVideoXEndpointArgs.parse_from_yaml(uargs_fpath)

    @override
    def prepare_dataset(self) -> None:
        self.endpoint_contract = self.uargs.resolved_contract()
        ...
        self.train_dataset = CogVideoXRolloutCacheDataset(
            ...
            manifest=self.uargs.endpoint_manifest,
            cache_dir=self.uargs.endpoint_cache_dir,
            expected_model_id=str(self.uargs.model_path),
            expected_scaling_factor=self.uargs.endpoint_expected_vae_scaling_factor,
            expected_posterior_mode=self.uargs.endpoint_expected_posterior_mode,
        ...
        self._endpoint_prompt_embedding = self.encode_text(self.uargs.endpoint_prompt).to("cpu")
        self.components.text_encoder.to("cpu")
        free_memory()
        self._write_resolved_training_contract(vae_scale=vae_scale, latent_shape=latent_shape)
    ...
    def collate_fn(self, samples: list[dict[str, Any]]) -> dict[str, Any]:
        prompt_embedding = self._endpoint_prompt_embedding.unsqueeze(0).expand(
            len(samples), -1, -1
        )
        return {
    ...
    def on_train_epoch_start(self, epoch: int) -> None:
        ...
        self._endpoint_epoch = int(epoch)
        if self.uargs.seed is not None:
            rank_epoch_seed = (
                int(self.uargs.seed)
                + self._endpoint_epoch * self.state.world_size
    ...
    def _prepare_training_batch(
        ...
        endpoint, frame_weights, provenance = construct_cached_endpoint(
            batch["cache_entries"],
            batch["sample_ids"],
            self.endpoint_contract,
            endpoint_seed=self.uargs.endpoint_seed,
            epoch=self._endpoint_epoch,
        )
        batch["encoded_videos"] = endpoint.permute(0, 2, 1, 3, 4).contiguous()
        batch["frame_weights"] = frame_weights
        batch["endpoint_provenance"] = tuple(item.to_dict() for item in provenance)
    ...
    def _add_training_noise(
        ...
        *,
        ...
        return clamp_endpoint_conditions(noisy, clean_latent, self.endpoint_contract)
    ...
    def _validate_model_contract(self, vae_scale: int) -> None:
        config = self.state.transformer_config
        actual_scaling_factor = float(self.components.vae.config.scaling_factor)
        expected = float(self.uargs.endpoint_expected_vae_scaling_factor)
        if abs(actual_scaling_factor - expected) > 1e-12:
            raise ValueError(
                "loaded VAE scaling factor disagrees with endpoint cache contract: "
                f"actual={actual_scaling_factor}, expected={expected}"
            )
        if vae_scale != 8 or int(config.in_channels) != 16 or int(config.patch_size) != 2:
            raise ValueError("EndpointRemover requires CogVideoX-5B latent/patch geometry")
        if config.patch_size_t is not None:
            raise ValueError("EndpointRemover requires CogVideoX with patch_size_t=None")
        if not config.use_rotary_positional_embeddings:
            raise ValueError("EndpointRemover requires native 3D rotary embeddings")
    ...
    def _write_resolved_training_contract(
        ...
        payload = {
            "schema_version": 1,
            "method": "EndpointRemover",
            "backbone": "CogVideoX-5B",
            "topology": self.endpoint_contract.to_dict(),
            "model_path": str(self.uargs.model_path),
            "data_root": str(self.uargs.data_root),
            "manifest": str(self.uargs.endpoint_manifest),
            "manifest_sha256": self._sha256(Path(self.uargs.endpoint_manifest)),
            "cache_dir": str(self.uargs.endpoint_cache_dir),
            "prompt": self.uargs.endpoint_prompt,
            "seed": self.uargs.seed,
            "diffusion_seed_policy": "seed_plus_epoch_times_world_size_plus_global_rank_v1",
            "endpoint_seed": self.uargs.endpoint_seed,
            "mask_selection": "deterministic_sample_epoch_hash_v1_matched_to_rr",
            "intermediate_rollout": None,
            "model_admission": {
        ...
        output_path = Path(self.uargs.output_dir) / "endpoint_training_contract.json"
        serialized = json.dumps(payload, indent=2, sort_keys=True) + "\n"
        if output_path.exists():
            if output_path.read_text(encoding="utf-8") != serialized:
                raise ValueError(f"existing endpoint contract disagrees with current run: {output_path}")
            return
        output_path.write_text(serialized, encoding="utf-8")


register("cogvideox-endpoint-t2v", "lora", CogVideoXEndpointT2VLoraTrainer)
