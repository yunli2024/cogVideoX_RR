"""CogKit-native CogVideoX sampling primitives for RolloutRemover.

The stock CogVideoX text-to-video call cannot keep two non-contiguous latent
states clean throughout denoising.  This module reuses the loaded CogVideoX
pipeline components and changes only that RolloutRemover sampling contract.
"""
...
from .contract import RolloutPreset


INFERENCE_POLICY = "cogkit_native_condition_clamped_rr_v1"
SEED_POLICY = "sha256_global_seed_plus_sample_id_v1"
...
def sample_seed(global_seed: int, sample_id: str) -> int:
    ...
    payload = f"cogvideox_rr_inference_v1\0{int(global_seed)}\0{sample_id}".encode()
...
def restore_clean_conditions(
    ...
    *,
    ...
    """Restore the two clean RR states without mutating the input tensor."""

    _validate_condition_pair(mask_condition, source_condition)
    if values.ndim != 5:
        raise ValueError("rollout tensor must have [B,F,C,h,w] shape")
    if values.shape[0] != mask_condition.shape[0] or values.shape[2:] != mask_condition.shape[1:]:
        raise ValueError("rollout tensor and condition latents disagree")
    if values.shape[1] != preset.total_frames:
        raise ValueError("rollout tensor frame count disagrees with preset")

    restored = values.clone()
    restored[:, preset.mask_condition_index] = mask_condition.to(
        device=values.device, dtype=values.dtype
    )
    restored[:, preset.source_condition_index] = source_condition.to(
        device=values.device, dtype=values.dtype
    )
    return restored
...
def zero_condition_prediction(
    ...
    """Match WAN-RR by suppressing updates predicted for clean states 0 and 2."""

    if prediction.ndim != 5 or prediction.shape[1] != preset.total_frames:
        raise ValueError("prediction must have preset-aligned [B,F,C,h,w] shape")
    result = prediction.clone()
    result[:, list(preset.clean_condition_indices)] = 0
    return result


def build_initial_rollout_latents(
    *,
    mask_condition: torch.Tensor,
    source_condition: torch.Tensor,
    preset: RolloutPreset,
    ...
    """Create `[mask, noise, source, noise...]` in CogVideoX tensor order."""
...
def sample_rollout_latents(
    ...
    *,
    ...
    """Run CogVideoX denoising while freezing RolloutRemover states 0 and 2."""
    ...
    for step_index, timestep in enumerate(timesteps):
        latents = restore_clean_conditions(
            latents,
            mask_condition=mask_condition,
            source_condition=source_condition,
            preset=preset,
        ...
        if do_cfg:
            ...
            prediction = unconditioned + current_guidance * (
                conditioned - unconditioned
            )
        prediction = zero_condition_prediction(prediction, preset)
        ...
        if isinstance(pipeline.scheduler, CogVideoXDPMScheduler):
            ...
            if old_pred_original_sample is not None:
                old_pred_original_sample = restore_clean_conditions(
                    old_pred_original_sample,
                    mask_condition=mask_condition,
                    source_condition=source_condition,
                    preset=preset,
...
@torch.no_grad()
def decode_rollout_states_individually(
    vae,
    latents: torch.Tensor,
) -> torch.Tensor:
    """Decode `[B,F,C,h,w]` through F independent temporal-length-one calls."""
